...
import net.jzx7.regios.bukkit.SpoutPlugin.GUI.RegionScreen4.ExToggle;
import net.jzx7.regios.bukkit.SpoutPlugin.GUI.RegionScreen5.PermToggle;
...
public class Screen_Listener implements Listener {
	...
	public static HashMap<SpoutPlayer, ArrayList<GenericLabel>> oldWidgets = new HashMap<SpoutPlayer, ArrayList<GenericLabel>>();
	...
	private void regionScreen1Listener(ButtonClickEvent evt, ScreenHolder sh) {
		...
		if (buttonID == generalProtection) {
			if (!r.canModify(RegiosConversions.getRegiosPlayer(sp))) {
	...
	private void regionScreen5Listener(ButtonClickEvent evt, ScreenHolder sh) {
		...
		if (button == cachea) {
			RegionScreen5.switchToggle(sp, PermToggle.CACHE_ADD, sh, r, evt.getButton(), false);
		}
		
		if (button == cacher) {
			RegionScreen5.switchToggle(sp, PermToggle.CACHE_REM, sh, r, evt.getButton(), false);
		}
